...
bool CSoundManager::add(wchar_t *filename, int *id)
{
	// Create a sound file
	...
	if (FAILED(hr = Create(&newSound, filename, DSBCAPS_CTRLVOLUME, GUID_NULL)))
...
bool CSoundManager::play(int id, bool bLooped)
{
	...
	LONG volume;
	m_soundVector[id]->GetFreeBuffer()->GetVolume(&volume);
